...
from detection_msgs.srv import GetImageAndBbox
...
class Yolov5Detector(Node):
    ...
    def intialize(self):
        ...
        self.compressed_input = bool(ti.topic_type == "sensor_msgs/msg/CompressedImage")
        ...
        self.get_logger().info('Subscribed to '+ str(self.input_image_topic))

        self.bb_srv = self.create_service(GetImageAndBbox, self.bb_service_name, self.bb_srv_callback)



    def input_image_callback(self, data):
        self._last_img_received = data


    def bb_srv_callback(self, req: GetImageAndBbox.Request, res: GetImageAndBbox.Response):
        """adapted from yolov5/detect.py"""
        if self.compressed_input:
            im = self.bridge.compressed_imgmsg_to_cv2(
                self._last_img_received,
        ...
        bounding_boxes.header = self._last_img_received.header
        bounding_boxes.image_header = self._last_img_received.header
        ...
        if len(det):
            ...
            res.image = self._last_img_received
...
def main(args=None):
    ...
    detector.input_image_topic = '/myumi_005/sensors/top_azure/rgb/image_raw/compressed'
    detector.bb_service_name = node_name + '/get_image_and_bbox'
